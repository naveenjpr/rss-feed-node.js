const vocabularyModel = require("../../models/vocabulary.Schema.js");
...
exports.view = async (request, response) => {
  try {
    ...
    const totalRecords = await vocabularyModel.countDocuments(condition);
    const result = await vocabularyModel.find(condition).sort({ _id: -1 });
...
exports.update = async (request, response) => {
  try {
    ...
    const record = await vocabularyModel.findById(id);
    ...
    const updated = await vocabularyModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true },
    );
