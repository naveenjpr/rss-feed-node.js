const phpModel = require("../../models/php.Schema.js");
...
exports.view = async (request, response) => {
  try {
    ...
    const totalRecords = await phpModel.countDocuments(condition);
    const result = await phpModel.find(condition).sort({ _id: -1 });
...
exports.update = async (request, response) => {
  try {
    ...
    const record = await phpModel.findById(id);
    ...
    const updated = await phpModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true },
    );
...
exports.delete = async (request, response) => {
  try {
    ...
    await phpModel.findByIdAndDelete(id);
