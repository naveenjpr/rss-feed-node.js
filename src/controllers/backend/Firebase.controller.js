const FirebaseModel = require("../../models/Firebase.Schema.js");
...
exports.view = async (request, response) => {
  try {
    ...
    const totalRecords = await FirebaseModel.countDocuments(condition);
    const result = await FirebaseModel.find(condition).sort({ _id: -1 });
...
exports.details = (request, response) => {
  ...
  FirebaseModel.findById(request.params.id)
    .then((result) => {
      if (result != "") {
        var res = {
          status: true,
...
exports.update = async (request, response) => {
  try {
    ...
    const record = await FirebaseModel.findById(id);
    ...
    const updated = await FirebaseModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true },
    );
...
exports.delete = async (request, response) => {
  try {
    ...
    if (!record) {
      return response.status(404).json({
        status: false,
        message: "Firebase note not found",
      });
    }
    ...
    await FirebaseModel.findByIdAndDelete(id);
