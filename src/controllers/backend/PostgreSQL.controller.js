...
exports.update = async (request, response) => {
  try {
    ...
    const record = await PostgreSQLModel.findById(id);
    ...
    const updated = await PostgreSQLModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true },
    );
...
exports.delete = async (request, response) => {
  try {
    const data = await PostgreSQLModel.findById(request.params.id);

    if (!data) {
      return response.status(404).json({
        status: false,
        message: "PostgreSQL notes not found",
      });
    }
    ...
    await PostgreSQLModel.findByIdAndDelete(request.params.id);

    return response.status(200).json({
      status: true,
      message: "PostgreSQL notes deleted successfully",
    });
  } catch (error) {
    return response.status(500).json({
      status: false,
