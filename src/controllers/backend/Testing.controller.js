const TestingModel = require("../../models/Testing.Schema");
...
exports.view = async (request, response) => {
  try {
    ...
    const totalRecords = await TestingModel.countDocuments(condition);
    const result = await TestingModel.find(condition).sort({ _id: -1 });
...
exports.details = (request, response) => {
  ...
  TestingModel.findById(request.params.id)
    .then((result) => {
      if (result != "") {
        var res = {
          status: true,
...
exports.update = async (request, response) => {
  try {
    ...
    const record = await TestingModel.findById(id);
    ...
    const updated = await TestingModel.findByIdAndUpdate(
      id,
      { $set: updateData },
      { new: true },
    );
...
exports.delete = async (request, response) => {
  try {
    const data = await TestingModel.findById(request.params.id);

    if (!data) {
      return response.status(404).json({
        status: false,
        message: "Testing  notes not found",
      });
    }
    ...
    return response.status(200).json({
      status: true,
      message: "Testing  notes deleted successfully",
    });
  } catch (error) {
    return response.status(500).json({
      status: false,
